...
def opt_reader(
    opt_path: str | os.PathLike | list[str | os.PathLike],
    /,
    filepre: str = "",
    titlepre: str = "",
    fill: bool = True,
    specname: dict | None = None,
    dump: bool = False,
) -> dict | None:
    ...
    # [ ] Redo this
    if isinstance(opt_path, (str, os.PathLike)) and ".json" in str(opt_path):
        files = [os.path.abspath(opt_path)]
        run_path = os.path.dirname(files[0])
    elif (
        not isinstance(opt_path, (str, os.PathLike))
        and hasattr(opt_path, "__iter__")
        and len(opt_path)
    ):
        ...
        if not opt_path:
            run_path = os.getcwd()
        elif isinstance(opt_path, (str, os.PathLike)):
    ...
    for file in files:
        ...
        if "deflist" in globals:
            data_types = {
                tipe: data_types.get(tipe, {}) for tipe in globals.pop("deflist")
            }
        ...
        for key, group in jason.items():
            ...
            group_item = (
                (
                    group.pop("group")
                    if group_type == "group"
                    else {group_type: group.pop(group_type)}
                )

...
def tri_bbox_prep(mesh, bbox, vals):
    ...
    xnodes = mesh.x
    ynodes = mesh.y
    xwide = (np.max(xnodes) - np.min(xnodes)) * 0.01
    ywide = (np.max(ynodes) - np.min(ynodes)) * 0.01
    ...
    trimmed = trigridtrim(
        mesh.triangles, np.vstack((xnodes, ynodes)).transpose(), adjustedbbox, vals=vals
    )
    if np.any(vals):
...
def trigridtrim(
    ...
    bbox_x = np.where(
        np.logical_and(barycenters[0] >= bbox[0], barycenters[0] <= bbox[2])
    )
    bbox_y = np.where(
        np.logical_and(barycenters[1] >= bbox[1], barycenters[1] <= bbox[3])
    )
    bboxindex = np.intersect1d(bbox_x, bbox_y)

    if not np.any(vals) and not returnindex:
        return elemcomps[bboxindex]  # ty: ignore[no-matching-overload]
...
def trigridcut(
    nodes: np.ndarray,
    bbox: list,
    elemcomps: np.ndarray | None = None,
    vals: np.ndarray | None = None,
    returnindex: bool = False,
) -> np.ndarray | list[np.ndarray]:
    ...
    if not np.any(vals) and not np.any(elemcomps) and not returnindex:
        return nodes[bboxindex, 0:3]  # ty: ignore[no-matching-overload]
    ...
    if np.any(elemcomps):  # ty: ignore[no-matching-overload]
        ...
        bboxelemcomp = elemcomps[
            elemcompbool[:, 0] & elemcompbool[:, 1] & elemcompbool[:, 2]
    ...
    if np.any(vals):
        returnlist.append(vals[bboxindex])  # ty: ignore[no-matching-overload,not-subscriptable]
    if returnindex:
        returnlist.append(bboxindex)
    return returnlist
...
def meshgridcut(
    mesh: np.ndarray, bbox: list, vals: np.ndarray | None = None
) -> np.ndarray | tuple[np.ndarray, np.ndarray]:
    ...
    if np.any(vals):
        bboxvals = np.delete(vals, dele, axis=0)  # ty: ignore[no-matching-overload]
    ...
    if np.any(vals):
        bboxvals = np.delete(bboxvals, dele, axis=1)  # ty: ignore[no-matching-overload]

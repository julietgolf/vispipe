...
def _getfunc(plm):
    ...
    func = getattr(
        importlib.import_module(".".join((readlist := plm.split("."))[:-1])),
        readlist[-1],
    )
    return func
...
def _writepdf(pngpath, width, height):
    ...
    file.save(
        os.path.join(os.path.dirname(os.path.dirname(pngpath)), f"pages/{pdfname}.pdf")
    )
    ...
    for key, item in plotsig.copy().items():
        if "*" in key:
            ...
            if item is not None:
                par += (
                    vals[int(item)]
                    if "vp:" not in str(item)
                    else tuple(kwargs.pop(item[3:], locals.get(item[3:])))
                )
            break
        del plotsig[key]
        ...
        if str(item).isdigit():
            item = vals[int(item)]
            if key == "rec" and recnumber is not None:
                item = item[recnumber]
            par += (item,)
            continue
            ...
            par += tuple(
                vals[int(i)]
                if "vp:" not in str(i)
                else kwargs.pop(i[3:], locals.get(item[3:]))
                for i in item
            )
            continue
        ...
        if rlow:
            if rhigh and isinstance(rlow, dict):
                for key, rhigh_item in rhigh.items():
                    ...
                    rlow_item = {
                        rli_key: rli_item
                        for rli_key, rli_item in rlow.get(key, {}).items()
                        if rli_key not in high
                    }
                    if rhigh_item and rlow_item:
                        rhigh[key] = _squash_level(
                            rhigh_item, rlow_item, readers_jason, plotters_jason
                        )
                    elif not rhigh_item:
                        rhigh[key] = rlow_item
...
def _pipeline(kwargs):
    ...
    try:
        ...
        logging.basicConfig(
            format="%(levelname)s: %(message)s", level=kwargs.pop("loglevel")
        )
        if "type" in kwargs:
            del kwargs["type"]
        ...
        if "table" in kwargs:
            plot: MPL_Figure = plot_api(
                ...
                subplots_kw=kwargs.pop("subplots_kw", {}),
            )
            ...
            tableargs, tablekwargs = _read_sig(
                table, vals, kwargs.copy(), tableargs, tablekwargs, locals(), recnumber
            )
            ...
            plot: MPL_Figure = plot_api(
                ...
                title=title,
                figsize=(width / dpi, height / dpi),
                layout=kwargs.pop("layout", "compressed"),
                subplots_kw=kwargs.pop("subplots_kw", {}),
            )
        ...
        if useback:
            ...
            if "subtitle" in plotsettings:
                plot.set_subtitle(plotsettings["subtitle"], ax=ax) if not isinstance(
                    plotsettings["subtitle"], dict
                ) else plot.set_subtitle(ax=ax, **plotsettings["subtitle"])

            if "xlabel" in plotsettings:
                plot.set_xlabel(plotsettings["xlabel"], ax=ax) if not isinstance(
                    plotsettings["xlabel"], dict
                ) else plot.set_xlabel(ax=ax, **plotsettings["xlabel"])

            if "ylabel" in plotsettings:
                plot.set_ylabel(plotsettings["ylabel"], ax=ax) if not isinstance(
                    plotsettings["ylabel"], dict
                ) else plot.set_ylabel(ax=ax, **plotsettings["ylabel"])

            if "xticks" in plotsettings:
                plot.set_xticks(plotsettings["xticks"], ax=ax) if not isinstance(
                    plotsettings["xticks"], dict
                ) else plot.set_xticks(ax=ax, **plotsettings["xticks"])

            if "yticks" in plotsettings:
                plot.set_yticks(plotsettings["yticks"], ax=ax) if not isinstance(
                    plotsettings["yticks"], dict
                ) else plot.set_yticks(ax=ax, **plotsettings["yticks"])

            if "aspect" in plotsettings:
                plot.set_aspect(plotsettings["aspect"], ax=ax) if not isinstance(
                    plotsettings["aspect"], dict
                ) else plot.set_aspect(ax=ax, **plotsettings["aspect"])

            if "grid" in plotsettings:
                plot.set_grid(plotsettings["grid"], ax=ax) if not isinstance(
                    plotsettings["grid"], dict
                ) else plot.set_grid(ax=ax, **plotsettings["grid"])

            if "bbox" in plotsettings:
                plot.set_bbox(plotsettings["bbox"], ax=ax) if not isinstance(
                    plotsettings["bbox"], dict
                ) else plot.set_bbox(ax=ax, **plotsettings["bbox"])
        ...
        if "sig" not in plotter:
            plotargs = (
                ((vals,) if not isinstance(vals, tuple) else vals)
                + kwargs.pop("mesh", ())
                + plotargs
            )
            plotkwargs.update(kwargs)
        else:
            plotargs, plotkwargs = _read_sig(
                plotter, vals, kwargs, plotargs, plotkwargs, locals(), recnumber
            )
        ...
        if useback and "cbar" in plotsettings:
            plot.cbar(
                ...
                **plotsettings.pop(
                ...
                if isinstance(plotsettings["cbar"], dict)
                else {},
            )

        pngpath = os.path.join(
            savedir,
            f"pngs/{os.path.basename(path).split('.')[0] + '-' if not pdf else ''}{pagenumber}.png",
        )
...
def vispipe(config, image=True, pdf=False, compress=False, loglevel=30):
    ...
    squash_level = partial(
        _squash_level, readers_jason=readers_jason, plotters_jason=plotters_jason
    )
    ...
    for key, settings in universal_jason.items():
        ...
        if "base" in settings:
            ...
            if "file_records" in settings:
                universal_jason[base]["recs"] = _parse_file_records(
                    universal_jason[base]
                )
    ...
    for i, (key, plot) in enumerate(plots_jason.items()):
        ...
        logging.debug("Setting references.")
        if plot is None:
            plot = {}
        """ if "type" in plot:
            globkey=plot["type"]
        ...
        for j, (rkey, rec) in enumerate(recs.items()):
            ...
            if not dif or "vals" in locplotdict and not len(dif) - 1:
                ...
                logging.error(
                    f"Keys {minreqs - set(locplotdict.keys())} are missing for plot {key}. Skipping."
                )
    ...
    try:
        if pdf:
            _orderpdf(os.path.join(savedir, savepdf))
    except Exception as e:
        logging.exception(f"{e}")
        if not image:
    ...
    if compress:
        ...
        if os.path.isfile(f"{pngpath}.tar.gz"):
            ...
            pathlib.Path(f"{pngpath}.tar.gz").unlink()

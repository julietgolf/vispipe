...
class MPL_Figure(_vispipe_backend_api):
    def __init__(
        ...
        if ax is None and not fig:
            fig, ax = plt.subplots(
                *(subplots if subplots else (1, 1)), figsize=figsize, **subplots_kw
            )
    ...
    def link_subplots(self, ax1, *ax2, sharex=True, sharey=True):
        ...
        for ax in child_axs:
            if sharex:
                ax1._shared_axes["x"].join(parent_ax, ax)
            if sharey:
                ax1._shared_axes["y"].join(parent_ax, ax)
    ...
    def contour(
        ...
        **conkwargs
        ...
        if fill:
            ...
            cm = ax.contour(mesh[0], mesh[1], vals, levels=levels, **conkwargs)
    ...
    def tricontour(
        ...
        **triconkwargs
        ...
        # Creates the filled contour to be plotted.
        if fill:
            cm = ax.tricontourf(mesh, vals, levels=levels, **triconkwargs)
        else:
            cm = ax.tricontour(mesh, vals, levels=levels, **triconkwargs)
    ...
    def cbar(
        self, mappable, cax=None, ax=None, label=None, label_kwargs={}, **colorbarkwargs
    ):
        """Calls plt.colorbar().
        ...
        **colorbarkwargs
        ...
        if label:
            cbar.ax.set_ylabel(
                label, **{**{"rotation": 90, "labelpad": 18}, **label_kwargs}
            )
        return cbar

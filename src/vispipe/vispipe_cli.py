from . import vispipe
from ._opt_reader import _opt_reader
...
def main():
    ...
    _vispipe_parser.add_argument('-i',"--image",action='store_false',help="Keepsssssss */pngs at the end of the run.",default=True)
    _vispipe_parser.add_argument('-p',"--pdf",action='store_true',help="Keep */pngs at the end of the run.",default=False)
    _vispipe_parser.add_argument('-c',"--compress",action='store_true',help="Compress */pngs to a .tar.gz. If -ic */pngs will be compressed and not deleted.",default=False)
    ...
    _vispipe_parser.add_argument("-f","--filepre",type=str,help="-o only: Common file prefix for all output files.",default="")
    _vispipe_parser.add_argument("-t","--titlepre",type=str,help="-o only: Common title prefix for all plots.",default="")
    _vispipe_parser.add_argument("-n","--nofill",action="store_false",help="-o only: Does not add missing datatypes to groups from deflist.",default=True)
    ...
    _vispipe_parser.add_argument("-d","--dump",action="store_true",help="-o only: Dumps opt into a opt.json.",default=False)
    ...
    if args.swap_settings:
        ...
        try:
            if args.opts:
                ...
                args.config=[_opt_reader(args.config,**{"filepre":args.filepre,"titlepre":args.titlepre,"fill":args.nofill,"specname":args.specname,"dump":args.dump})]
                print(args.config)
